...
These demonstrations consume a local AgentGuard core install. The core repository is private; a random GitHub visitor cannot always run the suite.

## Prerequisites

- macOS or Linux
- Python 3.11+
- Access to the AgentGuard core checkout used with this showcase (validated line: `0.2.0rc3`)
- Sibling layout, or `AGENTGUARD` pointing at a working CLI

```text
<parent>/
  agentguard/              # core
  agentguard-showcase/     # this repo
```
...
`bootstrap.sh --install` creates a **showcase-local** virtualenv and installs the sibling core in editable mode. It does not modify core source files.
...
### Refund mutation — `demo/refund-agent`
...
### Replay prevention — `demo/replay-prevention`
...
```bash
./demo/refund-agent/run.sh
./demo/replay-prevention/run.sh
./demo/mcp-security/run.sh
```

## Inspecting generated state

```bash
./demo/refund-agent/run.sh --keep-workspace
```
